...
void run_tests(bool debug = false)
{
    ...
    // Initial Guess
    std::vector<std::vector<double>> A{{0.7, 0.3}, {0.3, 0.7}};  // Transition Matrix
    std::vector<double> S = {0.4, 0.6};                          // Start probabilities
    std::vector<std::vector<double>> E{{0.699, 0.301}, {0.299, 0.701}};  // Emission Matrix

    size_t T = 30;                                // Time Horizon
    size_t numZeros = 28;                              // Number of zeros
    ...
    size_t num_valid = 10;                         // Number of sets of observations for validation
                                                  // Chose to give 1000 time steps
    double perturbParam = 0.75;                   // How much the parameters are perturbed
    ...
    for(size_t i = 0; i < numObsVec.size(); ++i) {
        ...
        size_t density_den = 0;
        const size_t density_num = numObs*testSize;
        ...
        for(size_t j = 0; j < testSize; ++j) {
            ...
            for(size_t k = 0; k < numObs; ++k) {
                ...
                originalCHMM.hmm.run(obs,hid);
                ++density_den;
                if(originalCHMM.constraint_oracle->operator()(hid)) {
                    obsVec[k] = obs;
                    hidVec[k] = hid;
                }
                else {
            ...
            //CHMM -- unconstrained
            std::cout << "Running unconstrained batch learning." << std::endl;
            ...
            //CHMM -- hardEM
            std::cout << "Running hardEM batch learning." << std::endl;
            ...
            //CHMM -- stochastic
            std::cout << "Running stochastic batch learning." << std::endl;
            ...
            //CHMM -- MIP
            std::cout << "Running MIP batch learning." << std::endl;
            ...
            MIP_error[i].push_back(std::max( matError(MIP_CHMM.hmm.getA(),trueA),
                std::max(vecError(MIP_CHMM.hmm.getS(),trueS), matError(MIP_CHMM.hmm.getE(), trueE))
            ));
        ...
        std::cout << "Density" << (double)density_num / ((double) density_den) << "\n\n";
        ...
        outputFile << (double)density_num / ((double) density_den) << "\n\n";
