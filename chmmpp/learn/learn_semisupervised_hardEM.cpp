                                 ...
                                 bool partialOracle, const Options &options)
{
    ...
    for (const auto &vec : supervisedObs) obs.push_back(vec);

    for (const auto &vec : unsupervisedObs) obs.push_back(vec);

    for (const auto &vec : supervisedHidden) hidden.push_back(vec);
    ...
    for (auto &vec : newA) normalize(vec);

    normalize(newS);

    for (auto &vec : newE) normalize(vec);
    ...
    while (true) {
        ...
        for (const auto &vec : unsupervisedObs) {
            ...
            aStarOracle(hmm, vec, tempHidden, tempLogProb, constraintOracle,
                        partialOracle);  // Could replace with IP or specific aStar
        ...
        for (auto &vec : ACounter) vec.resize(H);
        for (auto &vec : ECounter) vec.resize(O);
        ...
        for (auto &vec : ACounter) normalize(vec);

        normalize(SCounter);

        for (auto &vec : ECounter) normalize(vec);
